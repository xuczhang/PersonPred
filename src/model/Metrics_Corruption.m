function [ accuracy, precision, recall, f1_score ] = Metrics_Corruption( S_arr, S_truth, n )
...
for i=1:obj_num
    S_i = S_arr{i};
    S_estimate = zeros(n, 1);
    S_estimate(S_i) = 1;
    stat = confusionmatStats(S_truth, S_estimate);
    f1_score = f1_score + stat.Fscore(2);
    precision = precision + stat.precision(2);
    recall = recall + stat.recall(2);
    accuracy = accuracy + stat.accuracy(2);
end

f1_score = f1_score / obj_num;
precision = precision / obj_num;
recall = recall / obj_num;
accuracy = accuracy / obj_num;
